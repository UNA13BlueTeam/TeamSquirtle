...
<?php
		...
		{	
			...
			while($printLineIndex < strlen($printLine))
			{
				...
				{	
					...
					if(getCourse($printLine, $printLineIndex, $lineNumber, $firstCourseOnLineFlag, $currentCourse) == false)
					{
					...
					else
					{
						...
						if($printLine[$printLineIndex] != " ")
						{
							...
							echo "Error on line $lineNumber.  Whitespace must separate elements on the line." . PHP_EOL;
						}
					}
				}	
				else
				{
					...
					echo "Error on line $lineNumber.  Courses in file must contain between 1 and 3 prerequisites." . PHP_EOL;
					$errorOnLine = true;
				}
			}
	...
	/**********FUNCTIONS*********/
	function getCourse($line, &$lineIndex, $lineNumber, &$firstCourseOnLineFlag, &$currentCourse)
	 ...
	 $endCourseNameIndex = 0;
		...
		//ERROR HANDLING 
			if((count($courseLetters) < 2) or (count($courseLetters) > 4))
			{
				...
				echo "Error on line $lineNumber. Course letters must be between 2 and 4 characters. <br>";
				return false;
			}
		...
		//COURSE LETTERS ARE VALID AND IMMEDIATELY FOLLOWED BY NUMBERS
			else
			{
				...
				if(count($courseNumbers) != 3)
				{
					...
					echo "Error on line $lineNumber.  Course number must be exactly 3 digits. <br>";
					return false;
				}
				...
				if(($courseNumberInt < 1) or ($courseNumberInt > 499))
				{
					...
					echo "Error on line $lineNumber.  Course number exceeds boundaries. Must be between 001 and 499. <br>";
					return false;
				}
				else
				{//COURSE NUMBER IS VALID
					...
					if(count($endCourseName) > 2)
					{
						...
						echo "Error on line $lineNumber.  String of characters following course number is too long. <br>";
						return false;
					}
					else
					{
